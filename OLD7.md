...
<div class="modal-overlay hidden" id="admin-modal">
  <div class="modal-card" style="max-width:380px;">
    ...
    <div class="modal-actions">
      <button class="btn-secondary gold" id="maint-save">Сохранить</button>
    </div>
  </div>
</div>
...
(function(){
  ...
  // ---------- maintenance mode (shared, best-effort via keyless public store) ----------
  var MAINT_BUCKET_KEY = 'tbfpumba_maint_bucket_id';
  function getMaintBucket(){ try{ return localStorage.getItem(MAINT_BUCKET_KEY); }catch(e){ return null; } }
  function setMaintBucket(id){ try{ localStorage.setItem(MAINT_BUCKET_KEY, id); }catch(e){} }
  function ensureMaintBucket(cb){
    var existing = getMaintBucket();
    if (existing){ cb(existing); return; }
    fetch('https://kvdb.io/', {method:'POST'}).then(function(r){ return r.text(); }).then(function(id){
      id = id.trim();
      if (id){ setMaintBucket(id); cb(id); } else { cb(null); }
    }).catch(function(){ cb(null); });
  }
  function fetchMaintenanceState(cb){
    var bucket = getMaintBucket();
    if (!bucket){ cb({on:false}); return; }
    fetch('https://kvdb.io/' + bucket + '/maintenance').then(function(r){
      if (!r.ok) throw 0; return r.json();
    }).then(function(data){ cb(data || {on:false}); }).catch(function(){ cb({on:false}); });
  }
  function saveMaintenanceState(on, message){
    ensureMaintBucket(function(bucket){
      if (!bucket){ showToast('Не удалось подключиться к хранилищу'); return; }
      fetch('https://kvdb.io/' + bucket + '/maintenance', {
        method:'PUT', body: JSON.stringify({on:on, message:message||''})
      }).then(function(){
        showToast(on ? 'Тех. перерыв включён' : 'Игра снова доступна всем');
      }).catch(function(){ showToast('Не удалось сохранить — проверь соединение'); });
    });
  }
