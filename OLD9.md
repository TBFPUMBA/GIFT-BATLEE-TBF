...
<style>
  :root{
    --bg-0:#050709; --bg-1:#0d1015; --bg-2:#14181f;
    --blue:#3b82f6; --blue-dim:#1e4d8f; --cta:#0ea5e9; --gold:#f2b84b;
    --bone:#eaf1fb; --tan:#7d92b8; --line:rgba(234,241,251,0.09);
    --c-common:#7d92b8; --c-rare:#3b82f6; --c-epic:#8b5cf6; --c-legend:#f2b84b; --c-mythic:#eaf1fb;
    --danger:#ef476f;
  }
  *{box-sizing:border-box;margin:0;padding:0;}
  body{background:var(--bg-0);color:var(--bone);font-family:'IBM Plex Mono',monospace;line-height:1.6;padding-bottom:86px;}
  ...
  .hamburger-btn{
    ...
    transition:border-color .2s ease, transform .2s ease;flex-shrink:0;
  }
  .hamburger-btn:hover{border-color:var(--blue);}
  .hamburger-btn span{width:16px;height:2px;background:var(--bone);border-radius:2px;transition:transform .2s ease, opacity .2s ease;}

  .side-menu-overlay{
    position:fixed;inset:0;background:rgba(5,7,9,0.7);z-index:80;
  ...
  .side-menu{
    ...
    transform:translateX(-100%);transition:transform .28s cubic-bezier(0.2,0.9,0.3,1.1);
    padding:20px 16px;display:flex;flex-direction:column;gap:6px;
  }
  .side-menu.active{transform:translateX(0);}
  .side-menu-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;}
  .side-menu-tg{
    display:flex;align-items:center;gap:10px;background:var(--bg-2);border:1px solid var(--line);
    border-radius:14px;padding:10px 12px;margin-bottom:18px;font-size:13px;color:var(--bone);
  }
  .side-menu-tg img{width:32px;height:32px;border-radius:50%;object-fit:cover;}
  .side-menu-avatar-ph{
    width:32px;height:32px;border-radius:50%;background:var(--bg-1);
    display:flex;align-items:center;justify-content:center;font-size:16px;
  }
  .side-menu-item{
    background:transparent;border:none;color:var(--bone);text-align:left;
    display:flex;align-items:center;gap:12px;padding:13px 12px;border-radius:12px;
    font-size:14px;transition:background .18s ease, transform .18s ease;
  }
  .side-menu-item:hover{background:var(--bg-2);transform:translateX(3px);}
  .side-menu-ico{font-size:18px;}
  ...
  .profile-card{
    ...
    border:1px solid var(--line);border-radius:22px;padding:28px 20px;
  ...
  .profile-avatar{
    ...
    background:var(--bg-2);border:2px solid var(--blue);
    display:flex;align-items:center;justify-content:center;font-size:34px;
    box-shadow:0 0 24px rgba(59,130,246,0.35);
  }
  .profile-avatar img{width:100%;height:100%;object-fit:cover;}
  .profile-name{font-family:'Anton',sans-serif;font-size:22px;margin-top:12px;text-transform:uppercase;}
  .profile-sub{font-size:12px;color:var(--tan);margin-top:4px;}
  .showcase-row{display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin-top:20px;}
  .showcase-slot{
    width:56px;height:56px;border-radius:16px;border:2px dashed var(--line);
    background:var(--bg-2);display:flex;align-items:center;justify-content:center;
    font-size:26px;cursor:pointer;transition:border-color .18s ease, transform .18s ease;
  }
  .showcase-slot:hover{transform:translateY(-3px);border-color:var(--blue);}
  .showcase-slot.filled{border-style:solid;border-color:var(--gold);box-shadow:0 0 14px rgba(242,184,75,0.35);}
  .showcase-hint{font-size:11px;color:var(--tan);margin-top:12px;}

  .picker-overlay{position:fixed;inset:0;background:rgba(5,7,9,0.85);z-index:60;display:none;align-items:center;justify-content:center;padding:20px;}
  .picker-overlay.active{display:flex;}
  .picker-card{background:var(--bg-1);border:1px solid var(--line);border-radius:22px;padding:22px;max-width:420px;width:100%;max-height:76vh;overflow:auto;}
  .picker-card h3{font-size:18px;text-transform:uppercase;margin-bottom:14px;}
  .picker-grid{display:grid;grid-template-columns:repeat(auto-fill, minmax(80px,1fr));gap:10px;}
  .picker-item{background:var(--bg-2);border:1px solid var(--line);border-radius:14px;padding:10px;text-align:center;cursor:pointer;font-size:26px;transition:border-color .18s ease, transform .18s ease;}
  .picker-item:hover{border-color:var(--blue);transform:translateY(-2px);}
  .picker-close-row{text-align:right;margin-top:14px;}
  ...
  body.compact .wrap{padding:0 18px;}
  body.compact .nav{padding:12px 18px;}
  body.compact .intro{padding:16px 0 8px;}
  body.compact .intro p{display:none;}
  body.compact section{padding-top:10px;padding-bottom:10px;}
  body.compact .bank-block{padding:14px;margin-bottom:12px;}
  body.compact .bank-block h3{margin-bottom:10px;}
  body.compact .gift-card{padding:12px 8px;}
  body.compact .gift-card .ico{font-size:32px;}
  body.compact .inv-grid{gap:8px;}
  body.compact .section-head{margin-bottom:14px;}
  body.compact .xp-bar-wrap{margin:6px 0 4px;}
  body.compact .ref-block{padding:12px;}

  footer{padding:26px 28px 40px;font-size:12px;color:var(--tan);border-top:1px solid var(--line);}
  .footnote{margin-top:36px;padding-top:20px;border-top:1px solid var(--line);font-size:12px;color:var(--tan);}

  /* BOTTOM TAB BAR */
  .tab-bar{
    position:fixed;left:0;right:0;bottom:0;z-index:40;
    display:flex;justify-content:space-around;align-items:stretch;
    background:rgba(13,16,21,0.94);backdrop-filter:blur(14px);
  ...
  .tab-indicator{
    ...
    border-radius:16px;
    transition:left .32s cubic-bezier(0.2,0.9,0.25,1.1), width .32s cubic-bezier(0.2,0.9,0.25,1.1);
    pointer-events:none;
  }
  .tab-btn{
  ...
  .tab-btn .tab-ico{font-size:21px;line-height:1;filter:grayscale(1);opacity:0.6;transition:filter .22s ease, opacity .22s ease, transform .22s ease, text-shadow .22s ease;}
  .tab-btn.active{color:var(--blue);font-weight:600;}
  .tab-btn.active .tab-ico{filter:grayscale(0);opacity:1;transform:translateY(-3px) scale(1.15);text-shadow:0 0 14px rgba(59,130,246,0.7);}
  .tab-btn:active{transform:scale(0.94);}

  /* LABORATORY */
  .lab-stage{display:flex;align-items:center;justify-content:center;gap:18px;margin:26px 0 18px;flex-wrap:wrap;}
  .lab-slot{
    width:130px;height:130px;border:2px dashed var(--line);border-radius:20px;
    background:var(--bg-1);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;
    cursor:pointer;transition:border-color .18s ease, transform .18s ease, background .18s ease;
    text-align:center;padding:10px;
  }
  .lab-slot:hover{border-color:var(--blue);transform:translateY(-3px);}
  .lab-slot.filled{border-style:solid;border-color:var(--blue);background:var(--bg-2);}
  .lab-slot-ico{font-size:40px;color:var(--tan);}
  .lab-slot.filled .lab-slot-ico{font-size:46px;color:var(--bone);animation:bob 3s ease-in-out infinite;}
  .lab-slot-label{font-size:11px;color:var(--tan);}
  .lab-plus{font-family:'Anton',sans-serif;font-size:30px;color:var(--tan);}
  .lab-odds{text-align:center;font-size:12px;color:var(--tan);min-height:18px;}
  .lab-odds b{color:var(--bone);}
  #lab-combine-btn{max-width:320px;margin:0 auto;}
  .lab-inv-pick{border-color:var(--blue) !important;}

  .lab-fx-overlay{
    position:fixed;inset:0;z-index:70;display:none;align-items:center;justify-content:center;
    background:rgba(5,7,9,0.92);
  }
  .lab-fx-overlay.active{display:flex;}
  .lab-fx-ico{position:absolute;font-size:64px;transition:transform 1s cubic-bezier(0.6,0,0.2,1), opacity 1s ease;}
  .lab-fx-burst{
    width:10px;height:10px;border-radius:50%;background:var(--blue);
    box-shadow:0 0 0 0 rgba(59,130,246,0.6);
    animation:labBurst 0.6s ease-out forwards;
    animation-play-state:paused;
  }
  @keyframes labBurst{
    0%{box-shadow:0 0 0 0 rgba(59,130,246,0.7);opacity:1;}
    100%{box-shadow:0 0 0 160px rgba(59,130,246,0);opacity:0;}
  }
  ...
  @media (max-width:420px){
    ...
    .tab-btn .tab-ico{font-size:19px;}
    .lab-slot{width:110px;height:110px;}
  }
</style>
</head>
<body>
...
<header class="nav">
  <button class="hamburger-btn" id="hamburger-btn" title="Меню" aria-label="Меню">
...
<aside class="side-menu" id="side-menu">
  <div class="side-menu-head">
    <div class="brand">TBF<span>PUMBA</span></div>
    <button class="modal-close" id="side-menu-close">✕</button>
  </div>
  <div class="side-menu-tg" id="side-menu-tg"></div>
  <button class="side-menu-item" data-menu-action="profile"><span class="side-menu-ico">👤</span> Профиль</button>
  <button class="side-menu-item" data-menu-action="settings"><span class="side-menu-ico">⚙️</span> Настройки</button>
  <button class="side-menu-item" data-menu-action="admin" id="side-menu-admin" style="display:none;"><span class="side-menu-ico">🔧</span> Админ-панель</button>
</aside>

<div class="wrap screen" id="screen-roulette">
  <div class="intro">
...
<div class="wrap screen" id="screen-lab" style="display:none;">
  <div class="section-head">
    <h2>🧪 Лаборатория</h2>
    <p>Соедини 2 гифта — получи шанс на что-то круче</p>
  </div>

  <div class="lab-stage">
    <div class="lab-slot" id="lab-slot-a" data-slot="a">
      <div class="lab-slot-ico">＋</div>
      <div class="lab-slot-label">Выбери гифт</div>
    </div>
    <div class="lab-plus">+</div>
    <div class="lab-slot" id="lab-slot-b" data-slot="b">
      <div class="lab-slot-ico">＋</div>
      <div class="lab-slot-label">Выбери гифт</div>
    </div>
  </div>

  <div class="lab-odds" id="lab-odds"></div>

  <div class="spin-row" style="margin-top:10px;">
    <button class="btn-primary" id="lab-combine-btn" disabled>Соединить · 80 ★</button>
  </div>

  <div class="section-head" style="margin-top:36px;">
    <h3 style="font-family:'Anton',sans-serif;font-size:20px;text-transform:uppercase;">Твои гифты</h3>
  </div>
  <div id="lab-empty" class="empty-state">Нет гифтов для соединения — крути рулетку</div>
  <div class="inv-grid" id="lab-inv-grid"></div>
</div>

<div class="wrap screen" id="screen-profile" style="display:none;">
  <div class="profile-card" id="profile-card">
    <div class="profile-avatar-wrap">
      <div class="profile-avatar" id="profile-avatar">👤</div>
    </div>
    <div class="profile-name" id="profile-name">Гость</div>
    <div class="profile-sub" id="profile-sub">Уровень 1</div>
    <div class="showcase-row" id="showcase-row"></div>
    <div class="showcase-hint">Витрина — приколи до 6 гифтов на свой профиль</div>
  </div>
  <section class="profile">
    ...
    <div class="footnote">TBFPUMBA © 2026 · игровая валюта, не имеет реальной стоимости</div>
  </section>
</div>

<section class="bank-section wrap screen" id="screen-bank" style="display:none;">
  <div class="section-head">
    <h2>Банк TBFPUMBA</h2>
  ...
  <div class="bank-block">
    <h3>Кейсы</h3>
    <div class="shop-grid" id="cases-grid"></div>
  </div>

  <div class="bank-block">
    <h3>Премиум-кейсы · только за монеты</h3>
    <div class="shop-grid" id="premium-cases-grid"></div>
  </div>

  <div class="bank-block">
    <h3>Паки</h3>
    <div class="shop-grid" id="packs-grid"></div>
  </div>

  <div class="bank-block">
    <h3>Премиум-паки · только за монеты</h3>
    <div class="shop-grid" id="premium-packs-grid"></div>
  </div>
...
<nav class="tab-bar" id="tab-bar">
  ...
  <button class="tab-btn active" data-screen="roulette"><span class="tab-ico">🎰</span><span class="tab-label">Рулетка</span></button>
  <button class="tab-btn" data-screen="lab"><span class="tab-ico">🧪</span><span class="tab-label">Лаборатория</span></button>
  <button class="tab-btn" data-screen="bank"><span class="tab-ico">🏦</span><span class="tab-label">Банк</span></button>
</nav>
...
<div class="modal-overlay hidden" id="admin-modal">
  <div class="modal-card" style="max-width:380px;">
    ...
    <div class="modal-name" style="margin-top:0;">🔧 Админ-панель</div>
    ...
    <div class="settings-row">
      <h4>Сообщение игрокам</h4>
      <input type="text" id="maint-message" placeholder="Скоро вернёмся..." style="width:100%;background:var(--bg-2);border:1px solid var(--line);border-radius:12px;color:var(--bone);padding:11px 14px;font-size:13px;">
    </div>
    <div class="modal-desc" style="margin-top:8px;">Откроет GitHub с уже готовым файлом — останется нажать зелёную «Commit changes».</div>
    <div class="modal-actions">
      <button class="btn-secondary gold" id="maint-save">Применить на GitHub</button>
    </div>
  </div>
</div>

<div class="modal-overlay hidden" id="settings-modal">
  <div class="modal-card" id="settings-card" style="max-width:380px; max-height:86vh; overflow-y:auto;">
    ...
    <div class="settings-row">
      <div class="toggle-row">
        <span>Уведомлять об окончании бустов</span>
        <label class="switch">
          <input type="checkbox" id="boost-alerts-toggle">
          <span class="switch-track"></span>
        </label>
      </div>
    </div>
    <div class="settings-row" style="margin-top:28px;border-top:1px solid var(--line);padding-top:20px;">
      <button class="btn-secondary" id="reset-progress-btn" style="width:100%;border-color:var(--danger);color:var(--danger);">Сбросить прогресс</button>
    </div>
  </div>
</div>
...
<div class="lab-fx-overlay" id="lab-fx-overlay"></div>

<div class="picker-overlay" id="picker-overlay">
  <div class="picker-card">
    <h3>Выбери гифт для витрины</h3>
    <div class="picker-grid" id="picker-grid"></div>
    <div class="picker-close-row"><button class="btn-secondary" id="picker-close">Закрыть</button></div>
  </div>
</div>
...
(function(){
  ...
  var GIFTS = [
    ...
    {id:'mushroom', name:'Мухомор',   ico:'🍄', rarity:'common', weight:15,  sell:38,  desc:'Найден в лесу удачи.'},
    {id:'ice',      name:'Лёд',       ico:'🧊', rarity:'rare',   weight:11,  sell:150, desc:'Не тает даже в жару.'},
    {id:'star',     name:'Звезда',    ico:'⭐', rarity:'epic',   weight:4,   sell:370, desc:'Упала прямо в коллекцию.'},
    {id:'circus',   name:'Цирк',      ico:'🎪', rarity:'epic',   weight:3.5, sell:400, desc:'Шоу продолжается.'},
    {id:'lion',     name:'Лев',       ico:'🦁', rarity:'legend', weight:1.3, sell:1020,desc:'Король без короны.'}
  ];
  var RARITY_LABEL = {common:'COMMON', rare:'RARE', epic:'EPIC', legend:'LEGENDARY', mythic:'MYTHIC'};
  var LEGACY_STORE_KEY = 'tbfpumba_gift_battle_v3';
  var TG_UID = null;
  try{
    if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initDataUnsafe && window.Telegram.WebApp.initDataUnsafe.user){
      TG_UID = window.Telegram.WebApp.initDataUnsafe.user.id;
    }
  }catch(e){}
  var STORE_KEY = LEGACY_STORE_KEY + (TG_UID ? ('_' + TG_UID) : '_guest');
  try{
    if (TG_UID && !localStorage.getItem(STORE_KEY) && localStorage.getItem(LEGACY_STORE_KEY)){
  ...
  function loadState(){
    try{
      ...
      if (raw){
        ...
        s.settings = s.settings || {accent:'blue', reduceMotion:false, sound:true, haptics:true, compact:false};
        s.settings.sound = s.settings.sound !== undefined ? s.settings.sound : true;
        s.settings.haptics = s.settings.haptics !== undefined ? s.settings.haptics : true;
        s.settings.compact = s.settings.compact || false;
        s.settings.spinCurrency = s.settings.spinCurrency || 'stars';
        s.settings.boostAlerts = s.settings.boostAlerts !== undefined ? s.settings.boostAlerts : true;
        s.boosts = s.boosts || {};
        s.tgProfile = s.tgProfile || null;
        s.tgFromMiniApp = s.tgFromMiniApp || false;
    ...
    return {balance:500, bankStars:0, coins:0, xp:0, inventory:{}, bankGifts:[], redeemedCodes:[], loan:null, settings:{accent:'blue', reduceMotion:false, sound:true, haptics:true, compact:false, spinCurrency:'stars', boostAlerts:true}, boosts:{}, tgProfile:null, redeemedGiftCodes:[], myRefCode:null, referredBy:null, showcase:[]};
  }
  ...
  function renderProfileCard(){
    ...
    if (state.tgProfile){
      ...
      avatarEl.innerHTML = p.photo_url ? '<img src="'+p.photo_url+'" alt="">' : '👤';
      nameEl.textContent = p.first_name + (p.last_name ? ' ' + p.last_name : '');
    } else {
      avatarEl.innerHTML = '👤';
      nameEl.textContent = 'Гость';
    }
    subEl.textContent = 'Уровень ' + levelFromXp(state.xp);

    state.showcase = state.showcase.filter(function(id){ return (state.inventory[id]||0) > 0; });
    var row = document.getElementById('showcase-row');
    row.innerHTML = '';
    for (var i=0;i<SHOWCASE_MAX;i++){
      var id = state.showcase[i];
      var slot = document.createElement('div');
      slot.className = 'showcase-slot' + (id ? ' filled' : '');
      if (id){
        var g = byId(id);
        slot.textContent = g ? g.ico : '＋';
        slot.title = g ? g.name + ' — нажми, чтобы убрать' : '';
        slot.addEventListener('click', function(idx){
          return function(){ state.showcase.splice(idx,1); saveState(); renderProfileCard(); };
        }(i));
      } else {
        slot.textContent = '＋';
        slot.addEventListener('click', function(idx){
          return function(){ openShowcasePicker(idx); };
        }(i));
      }
  ...
  function openShowcasePicker(slotIdx){
    ...
    if (ids.length === 0){
      pickerGrid.innerHTML = '<div class="empty-state" style="grid-column:1/-1;">Нет гифтов — крути рулетку</div>';
    } else {
      ids.forEach(function(id){
        var g = byId(id); if (!g) return;
        var item = document.createElement('div');
        item.className = 'picker-item';
        item.textContent = g.ico;
        item.title = g.name;
        item.addEventListener('click', function(){
          state.showcase[slotIdx] = id;
          saveState(); renderProfileCard();
          closeShowcasePicker();
        });
  ...
  function closeShowcasePicker(){ pickerOverlay.classList.remove('active'); }
  document.getElementById('picker-close').addEventListener('click', closeShowcasePicker);
  pickerOverlay.addEventListener('click', function(e){ if (e.target === pickerOverlay) closeShowcasePicker(); });
  ...
  var spinning = false;
  var spinCurrency = (state.settings && state.settings.spinCurrency) || 'stars';
  ...
  function updateSpinButtonLabels(){
    document.querySelectorAll('.curr-btn').forEach(function(b){
      b.classList.toggle('active', b.getAttribute('data-currency') === spinCurrency);
    });
    spinButtons.forEach(function(b){
      var count = parseInt(b.getAttribute('data-count'),10);
      var cost = spinCost(count);
  ...
  document.querySelectorAll('.curr-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      state.settings.spinCurrency = spinCurrency;
      saveState();
  ...
  function openModal(g, justWon){
    ...
    startSpecial(g.id);
    if (justWon){
      playRaritySound(g.rarity);
      var hapticPattern = {
        common:[12], rare:[14,26,14], epic:[18,32,18,32,18],
        legend:[24,40,24,40,24,40,24], mythic:[30,50,30,50,30,50,30,50,30]
      }[g.rarity] || [12];
  ...
  function openBatchModal(results){
    ...
    var bestRarity = results.reduce(function(best, r){
      return rarityIdx(r.rarity) > rarityIdx(best) ? r.rarity : best;
    }, 'common');
    playRaritySound(bestRarity);
    vibrate([16,28,16]);
  ...
  function mkMat(color, opts){
    ...
    return new THREE.MeshPhysicalMaterial(Object.assign({
      color:color, metalness:0.5, roughness:0.26,
      clearcoat:0.8, clearcoatRoughness:0.15, reflectivity:0.75
    }, opts));
  }
  ...
  function buildCactusGroup(){
    ...
    var pot = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.65, 0.55, 20), potMat);
    ...
    var body = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.42, 1.6, 18), greenMat);
    body.position.y = -0.25; group.add(body);
    var topCap = new THREE.Mesh(new THREE.SphereGeometry(0.32, 18,18), greenMat);
    topCap.position.y = 0.55; group.add(topCap);
    function arm(x, rotZ){
      var g = new THREE.Group();
      var seg = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.16, 0.7, 16), greenMatDark);
      seg.position.set(0, 0.35, 0); g.add(seg);
      var cap = new THREE.Mesh(new THREE.SphereGeometry(0.13, 18,18), greenMatDark);
    ...
    group.add(arm(0.45, -Math.PI*0.18));
    var flower = new THREE.Mesh(new THREE.SphereGeometry(0.12, 18,18), flowerMat);
  ...
  function buildDemonGroup(){
    ...
    function horn(x){
      var h = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.4, 16), hornMat);
    ...
    function eye(x){
      var e = new THREE.Mesh(new THREE.SphereGeometry(0.05, 18,18), eyeMat);
      e.position.set(x, 0.58, 0.32);
      return e;
    }
    group.add(eye(-0.13), eye(0.13));
    var tail = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.55, 16), skinMat);
  ...
  function buildPhoenixGroup(){
    ...
    var beak = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.22, 16), mkMat(0xffd27a));
    ...
    function tailF(rot){
      var t = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.75, 14), tailMat);
  ...
  function buildRose(){
    ...
    var stem = new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.06,1.5,18), stemMat);
    ...
    for (var p=0;p<5;p++){
      var petal = new THREE.Mesh(new THREE.SphereGeometry(0.22,18,18), petalMat);
  ...
  function buildBear(){
    ...
    var nose = new THREE.Mesh(new THREE.SphereGeometry(0.05,18,18), mkMat(0x1a120c)); nose.position.set(0,0.46,0.48); g.add(nose);
    [-0.28,0.28].forEach(function(x){
      var ear = new THREE.Mesh(new THREE.SphereGeometry(0.14,18,18), m);
      ear.position.set(x,0.85,0.05); g.add(ear);
      var arm = new THREE.Mesh(new THREE.SphereGeometry(0.18,18,18), m);
      arm.position.set(x*1.8,-0.25,0.1); g.add(arm);
      var leg = new THREE.Mesh(new THREE.SphereGeometry(0.2,18,18), m);
  ...
  function buildBalloon(){
    ...
    var knot = new THREE.Mesh(new THREE.ConeGeometry(0.08,0.14,16), m);
  ...
  function buildBone(){
    ...
    var shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.14,0.14,1.3,20), m);
  ...
  function buildCake(){
    ...
    var candle = new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,0.4,16), mkMat(0x5fd4ff));
    candle.position.y = 0.45; g.add(candle);
    var flame = new THREE.Mesh(new THREE.ConeGeometry(0.07,0.18,16), mkMat(0xffb347,{emissive:0xff8a1a, emissiveIntensity:0.8}));
  ...
  function buildRing(){
    ...
    [-0.14,0.14].forEach(function(x){
      var prong = new THREE.Mesh(new THREE.ConeGeometry(0.04,0.18,14), mkMat(0xdba514,{metalness:0.85}));
  ...
  function buildJoystick(){
    ...
    var stick = new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.07,0.75,18), mkMat(0x556077));
    ...
    [[-0.25,-0.42],[0.05,-0.5]].forEach(function(p){
      var btn = new THREE.Mesh(new THREE.CylinderGeometry(0.08,0.08,0.06,20), mkMat(0xffd166));
  ...
  function buildRocket(){
    ...
    var flame = new THREE.Mesh(new THREE.ConeGeometry(0.2,0.5,20), mkMat(0xffb347,{emissive:0xff6a1a, emissiveIntensity:0.7}));
  ...
  function buildCrown(){
    ...
    for (var i=0;i<5;i++){
      var a = (i/5)*Math.PI*2;
      var spike = new THREE.Mesh(new THREE.ConeGeometry(0.14,0.5,18), m);
      spike.position.set(Math.cos(a)*0.45,0.4,Math.sin(a)*0.45); g.add(spike);
      var gem = new THREE.Mesh(new THREE.SphereGeometry(0.06,18,18), mkMat(0x9be8ff,{emissive:0x9be8ff, emissiveIntensity:0.5}));
  ...
  function buildSnake(){
    ...
    for (var i=0;i<segCount;i++){
      var t = i/(segCount-1);
      var s = new THREE.Mesh(new THREE.SphereGeometry(0.24*(1-t*0.55),18,18), m);
      s.position.set(Math.sin(t*Math.PI*1.6)*0.55, -0.7+t*1.3, Math.cos(t*Math.PI*1.6)*0.2);
      g.add(s);
    }
    var head = new THREE.Mesh(new THREE.ConeGeometry(0.13,0.3,18), mkMat(0x1c5c30));
  ...
  function buildAnchor(){
    ...
    var shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.07,0.07,1.3,18), m);
  ...
  function buildCup(){
    ...
    var stem = new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.1,0.55,20), m);
  ...
  function buildEagle(){
    ...
    var beak = new THREE.Mesh(new THREE.ConeGeometry(0.07,0.22,16), mkMat(0xf2b84b));
    ...
    var tail = new THREE.Mesh(new THREE.ConeGeometry(0.2,0.5,18), bodyMat);
  ...
  function buildUnicorn(){
    ...
    var body = new THREE.Mesh(new THREE.CylinderGeometry(0.28,0.32,1,20), m);
    body.rotation.z = Math.PI/2; body.position.y = 0.05; g.add(body);
    var neck = new THREE.Mesh(new THREE.CylinderGeometry(0.16,0.2,0.55,18), m);
    ...
    var horn = new THREE.Mesh(new THREE.ConeGeometry(0.05,0.4,16), mkMat(0xf2b84b,{metalness:0.7, emissive:0xf2b84b, emissiveIntensity:0.3}));
    horn.position.set(0.85,0.95,0); horn.rotation.z = -0.3; g.add(horn);
    for (var m1=0;m1<5;m1++){
      var mane = new THREE.Mesh(new THREE.ConeGeometry(0.06,0.22,14), mkMat(0xff8fc0));
      mane.position.set(0.6-m1*0.13,0.6-m1*0.02,0.02);
      mane.rotation.z = 1.3; g.add(mane);
    }
    [[-0.4,-0.55],[0,-0.55],[0.4,-0.55]].forEach(function(p){
      var leg = new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.06,0.5,16), m);
      leg.position.set(p[0],p[1],0); g.add(leg);
    });
    var tail = new THREE.Mesh(new THREE.ConeGeometry(0.1,0.5,16), mkMat(0xff8fc0));
  ...
  function buildPizza(){
    ...
    for (var i=0;i<6;i++){
      var top = new THREE.Mesh(new THREE.SphereGeometry(0.09,18,18), mkMat(toppingColors[i%3]));
  ...
  function buildLollipop(){
    ...
    var stick = new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.9,18), mkMat(0xf2ead9));
  ...
  function buildSunglasses(){
    ...
    [-0.34,0.34].forEach(function(x){
      ...
      var arm = new THREE.Mesh(new THREE.CylinderGeometry(0.025,0.025,0.55,16), m);
      arm.rotation.y = Math.PI/2; arm.rotation.z = 0.15*(x>0?1:-1);
      arm.position.set(x + (x>0?0.5:-0.5), 0, -0.15); g.add(arm);
    });
    var bridge = new THREE.Mesh(new THREE.CylinderGeometry(0.025,0.025,0.2,16), m);
  ...
  function buildButterfly(){
    var g = new THREE.Group();
    var body = new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.04,0.7,16), mkMat(0x1c1c22));
    g.add(body);
    ...
    [-0.06,0.06].forEach(function(x){
      var ant = new THREE.Mesh(new THREE.CylinderGeometry(0.012,0.012,0.3,14), mkMat(0x1c1c22));
  ...
  function buildTrident(){
    ...
    var shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.06,1.6,20), m);
    shaft.position.y = -0.2; g.add(shaft);
    var crossbar = new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.7,18), m);
    crossbar.rotation.z = Math.PI/2; crossbar.position.y = 0.5; g.add(crossbar);
    [-0.35,0,0.35].forEach(function(x){
      var prong = new THREE.Mesh(new THREE.ConeGeometry(0.06,0.55,18), m);
  ...
  function buildUfo(){
    ...
    for (var i=0;i<8;i++){
      var a = (i/8)*Math.PI*2;
      var light = new THREE.Mesh(new THREE.SphereGeometry(0.06,18,18), mkMat(0xf2b84b,{emissive:0xf2b84b, emissiveIntensity:0.8}));
  ...
  function buildDragon(){
    ...
    var belly = new THREE.Mesh(new THREE.ConeGeometry(0.25,1.1,20), bellyMat);
    ...
    var snout = new THREE.Mesh(new THREE.ConeGeometry(0.14,0.35,18), scaleMat);
    snout.rotation.x = Math.PI/2; snout.position.set(0,0.65,0.32); g.add(snout);
    [-0.12,0.12].forEach(function(x){
      var horn = new THREE.Mesh(new THREE.ConeGeometry(0.05,0.28,16), mkMat(0xeaf1fb));
      horn.position.set(x,0.98,0.05); horn.rotation.z = x>0?-0.3:0.3; g.add(horn);
      var eye = new THREE.Mesh(new THREE.SphereGeometry(0.045,18,18), mkMat(0xff2a2a,{emissive:0xff2a2a, emissiveIntensity:1}));
    ...
    var tail = new THREE.Mesh(new THREE.ConeGeometry(0.12,0.9,18), scaleMat);
  ...
  function buildMushroom(){
    var g = new THREE.Group();
    var capMat = mkMat(0xc11f3d, {roughness:0.4});
    var stemMat = mkMat(0xf2ead9, {roughness:0.55});
    var cap = new THREE.Mesh(new THREE.SphereGeometry(0.55,18,18,0,Math.PI*2,0,Math.PI/1.8), capMat);
    cap.position.y = 0.15; g.add(cap);
    var stem = new THREE.Mesh(new THREE.CylinderGeometry(0.16,0.22,0.75,16), stemMat);
    stem.position.y = -0.35; g.add(stem);
    for (var i=0;i<7;i++){
      var dot = new THREE.Mesh(new THREE.SphereGeometry(0.06,10,10), mkMat(0xffffff));
      var a = (i/7)*Math.PI*2, r = 0.25+Math.random()*0.2;
      dot.position.set(Math.cos(a)*r, 0.32+Math.random()*0.08, Math.sin(a)*r);
      g.add(dot);
    }
    return addBase(g, -0.95);
  }

  function buildIce(){
    var g = new THREE.Group();
    var m = mkMat(0x8fd8f0, {transparent:true, opacity:0.55, roughness:0.05, metalness:0.1, emissive:0x5fd4ff, emissiveIntensity:0.2});
    var cube = new THREE.Mesh(new THREE.OctahedronGeometry(0.6,0), m);
    cube.scale.set(1,1.15,1); g.add(cube);
    var inner = new THREE.Mesh(new THREE.OctahedronGeometry(0.32,0), mkMat(0xeaf1fb, {emissive:0xbfe9ff, emissiveIntensity:0.6}));
    g.add(inner);
    return addBase(g, -0.95);
  }
  ...
  function buildStar(){
    var g = new THREE.Group();
    var m = mkMat(0xf2b84b, {emissive:0xf2b84b, emissiveIntensity:0.45, roughness:0.25});
    var core = new THREE.Mesh(new THREE.OctahedronGeometry(0.42,0), m);
    g.add(core);
    for (var i=0;i<5;i++){
      var a = (i/5)*Math.PI*2;
      var spike = new THREE.Mesh(new THREE.ConeGeometry(0.13,0.55,14), m);
  ...
  function buildCircus(){
    var g = new THREE.Group();
    var stripeA = mkMat(0xc11f3d, {roughness:0.5});
    var stripeB = mkMat(0xeaf1fb, {roughness:0.5});
    var tent = new THREE.Mesh(new THREE.ConeGeometry(0.75,1.1,16), stripeA);
    tent.position.y = 0.1; g.add(tent);
    var band = new THREE.Mesh(new THREE.ConeGeometry(0.78,0.18,16,1,true), stripeB);
    band.position.y = -0.35; g.add(band);
    var pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03,0.03,0.35,10), mkMat(0xf2b84b));
    pole.position.y = 0.78; g.add(pole);
    var flag = new THREE.Mesh(new THREE.ConeGeometry(0.14,0.2,8), stripeA);
    flag.rotation.z = Math.PI/2; flag.position.set(0.12,0.9,0); g.add(flag);
    return addBase(g, -0.85);
  }

  function buildLion(){
    var g = new THREE.Group();
    var faceMat = mkMat(0xe0a94a, {roughness:0.5});
    var maneMat = mkMat(0x9a5a1a, {roughness:0.6});
    var mane = new THREE.Mesh(new THREE.SphereGeometry(0.5,18,18), maneMat);
    mane.scale.set(1,0.95,1); g.add(mane);
    var face = new THREE.Mesh(new THREE.SphereGeometry(0.32,16,16), faceMat);
    face.position.z = 0.1; g.add(face);
    var snout = new THREE.Mesh(new THREE.SphereGeometry(0.14,12,12), mkMat(0xf2ead9));
    snout.position.set(0,-0.08,0.36); g.add(snout);
    var nose = new THREE.Mesh(new THREE.SphereGeometry(0.05,8,8), mkMat(0x1a120c));
    nose.position.set(0,-0.02,0.48); g.add(nose);
    [-0.16,0.16].forEach(function(x){
      var ear = new THREE.Mesh(new THREE.SphereGeometry(0.1,10,10), maneMat);
      ear.position.set(x,0.42,0.05); g.add(ear);
      var eye = new THREE.Mesh(new THREE.SphereGeometry(0.04,8,8), mkMat(0x1a120c));
      eye.position.set(x*0.8,0.08,0.34); g.add(eye);
    });
    return addBase(g, -0.85);
  }
  ...
  var GIFT_BUILDERS = {
    ...
    mushroom:buildMushroom, ice:buildIce, star:buildStar, circus:buildCircus, lion:buildLion
  };
  ...
  var sKey = new THREE.DirectionalLight(0xffffff, 2.9); sKey.position.set(3.2,3.6,4.2); specialScene.add(sKey);
  ...
  var sTop = new THREE.PointLight(0xffffff, 1.2, 14); sTop.position.set(0,3.2,0.5); specialScene.add(sTop);
  var sBelow = new THREE.PointLight(0x8b5cf6, 0.6, 12); sBelow.position.set(0,-1.6,1.5); specialScene.add(sBelow);
  var sAccent = new THREE.PointLight(0xffffff, 1.6, 16); sAccent.position.set(-1.6,2.2,3); specialScene.add(sAccent);

  var RARITY_GLOW = {common:'125,146,184', rare:'59,130,246', epic:'139,92,246', legend:'242,184,75', mythic:'234,241,251'};
  var stageGlowCanvas = document.createElement('canvas');
  stageGlowCanvas.width = 512; stageGlowCanvas.height = 512;
  var stageGlowCtx = stageGlowCanvas.getContext('2d');
  var stageGlowTex = new THREE.CanvasTexture(stageGlowCanvas);
  var stageGlowMesh = new THREE.Mesh(
    new THREE.PlaneGeometry(7,7),
    new THREE.MeshBasicMaterial({map:stageGlowTex, transparent:true, depthWrite:false})
  );
  stageGlowMesh.position.z = -2.6;
  specialScene.add(stageGlowMesh);
  function paintStageGlow(rarity){
    var c = RARITY_GLOW[rarity] || RARITY_GLOW.common;
    stageGlowCtx.clearRect(0,0,512,512);
    var grad = stageGlowCtx.createRadialGradient(256,256,10,256,256,256);
    grad.addColorStop(0, 'rgba('+c+',0.55)');
    grad.addColorStop(0.5, 'rgba('+c+',0.16)');
    grad.addColorStop(1, 'rgba('+c+',0)');
    stageGlowCtx.fillStyle = grad;
    stageGlowCtx.fillRect(0,0,512,512);
    stageGlowTex.needsUpdate = true;
    sAccent.color.setStyle('rgb('+c+')');
    sRim.color.setStyle('rgb('+c+')');
    sparkMat.color.setStyle('rgb('+c+')');
  }
  ...
  function specialLoop(){
    ...
    if (!reduced){
      var revealT = Math.min(1, (performance.now()-revealStart)/650);
      var ease = easeOutCubic(revealT);
      specialCamera.position.z = 8.2 + (6.4-8.2)*ease;
      specialCamera.position.y = 1.25 + (0.55-1.25)*ease;
  ...
  var currentScreen = 'roulette';
  function switchScreen(name){
    if (name === currentScreen || !screens[name]) return;
    overlay.classList.add('active');
    setTimeout(function(){
      Object.keys(screens).forEach(function(k){ screens[k].style.display = (k===name) ? '' : 'none'; });
      currentScreen = name;
      document.querySelectorAll('.tab-btn').forEach(function(b){
        b.classList.toggle('active', b.getAttribute('data-screen') === name);
      });
      moveTabIndicator();
      if (name === 'bank'){ chargeBankUpkeep(); renderTop(); renderBank(); }
  ...
  // ---------- laboratory (fusion) ----------
  var RARITY_ORDER = ['common','rare','epic','legend','mythic'];
  var LAB_COST = 80;
  function rarityIdx(r){ return RARITY_ORDER.indexOf(r); }
  function fusionDistribution(gA, gB){
    var avg = (rarityIdx(gA.rarity) + rarityIdx(gB.rarity)) / 2;
    var target = Math.min(4, Math.round(avg) + 1);
    var dist = {};
    function add(idx, w){ if (idx>=0 && idx<=4){ dist[idx] = (dist[idx]||0) + w; } }
    add(target, 55);
    add(target-1, 25);
    add(target+1, 15);
    add(target-2, 3);
  ...
  function pickFusionRarity(gA, gB){
    var dist = fusionDistribution(gA, gB);
    var idxs = Object.keys(dist).map(Number);
    var total = idxs.reduce(function(s,k){ return s+dist[k]; }, 0);
    var r = Math.random()*total;
    for (var i=0;i<idxs.length;i++){ r -= dist[idxs[i]]; if (r<=0) return RARITY_ORDER[idxs[i]]; }
    return RARITY_ORDER[idxs[idxs.length-1]];
  }
  function pickGiftOfRarity(rarity){
    var pool = GIFTS.filter(function(g){ return g.rarity === rarity; });
    if (pool.length === 0) return GIFTS[0];
    var tw = pool.reduce(function(s,g){ return s+g.weight; }, 0);
    var r = Math.random()*tw;
    for (var i=0;i<pool.length;i++){ r -= pool[i].weight; if (r<=0) return pool[i]; }
    return pool[pool.length-1];
  }

  var labSlots = {a:null, b:null};
  ['a','b'].forEach(function(key){
    document.getElementById('lab-slot-'+key).addEventListener('click', function(){
      if (labSlots[key]){ labSlots[key] = null; renderLab(); }
    });
  });

  function renderLabSlots(){
    ['a','b'].forEach(function(key){
      var el = document.getElementById('lab-slot-'+key);
      var id = labSlots[key];
      if (id){
        var g = byId(id);
        el.classList.add('filled');
        el.innerHTML = '<div class="lab-slot-ico">'+g.ico+'</div><div class="lab-slot-label">'+g.name+'</div>';
      } else {
        el.classList.remove('filled');
        el.innerHTML = '<div class="lab-slot-ico">＋</div><div class="lab-slot-label">Выбери гифт</div>';
      }
    });
    var oddsEl = document.getElementById('lab-odds');
    var btn = document.getElementById('lab-combine-btn');
    btn.textContent = 'Соединить · ' + LAB_COST + ' ★';
    if (labSlots.a && labSlots.b){
      var gA = byId(labSlots.a), gB = byId(labSlots.b);
      var dist = fusionDistribution(gA, gB);
      var idxs = Object.keys(dist).map(Number).sort(function(x,y){ return x-y; });
      var total = idxs.reduce(function(s,k){ return s+dist[k]; }, 0);
      oddsEl.innerHTML = 'Шанс: ' + idxs.map(function(k){
        return RARITY_LABEL[RARITY_ORDER[k]].toLowerCase() + ' <b>' + Math.round(dist[k]/total*100) + '%</b>';
      }).join(' · ');
      btn.disabled = state.balance < LAB_COST;
    } else {
      oddsEl.textContent = 'Выбери два гифта снизу, чтобы увидеть шансы';
      btn.disabled = true;
    }
  }

  function validateLabSlots(){
    var seen = {};
    ['a','b'].forEach(function(key){
      var id = labSlots[key];
      if (!id) return;
      seen[id] = (seen[id]||0) + 1;
      var have = state.inventory[id] || 0;
      if (seen[id] > have){ labSlots[key] = null; }
    });
  }
  function renderLab(){
    validateLabSlots();
    var ids = Object.keys(state.inventory).filter(function(k){ return state.inventory[k] > 0; });
    var grid = document.getElementById('lab-inv-grid');
    var empty = document.getElementById('lab-empty');
    grid.innerHTML = '';
    if (ids.length === 0){ empty.style.display = 'block'; grid.style.display = 'none'; }
    else{
      empty.style.display = 'none'; grid.style.display = 'grid';
      ids.forEach(function(id){
        var g = byId(id); if (!g) return;
        var used = (labSlots.a===id?1:0) + (labSlots.b===id?1:0);
        var remaining = state.inventory[id] - used;
        var card = document.createElement('div');
        card.className = 'gift-card rar-' + g.rarity + (used>0 ? ' lab-inv-pick' : '');
        card.innerHTML = '<div class="count">x' + remaining + '</div><div class="ico">' + g.ico + '</div><div class="name">' + g.name + '</div>';
        card.addEventListener('click', function(){
          if (remaining <= 0){ showToast('Больше нет в наличии'); return; }
          if (!labSlots.a){ labSlots.a = id; }
          else if (!labSlots.b){ labSlots.b = id; }
          else { labSlots.a = id; labSlots.b = null; }
          renderLab();
        });
        grid.appendChild(card);
      });
  ...
  function playLabFusion(gA, gB, result){
    var fx = document.getElementById('lab-fx-overlay');
    fx.innerHTML = '';
    fx.classList.add('active');
    var iconA = document.createElement('div');
    iconA.className = 'lab-fx-ico'; iconA.textContent = gA.ico;
    iconA.style.left = '32%'; iconA.style.top = '48%'; iconA.style.transform = 'translate(-50%,-50%)'; iconA.style.opacity = '1';
    var iconB = document.createElement('div');
    iconB.className = 'lab-fx-ico'; iconB.textContent = gB.ico;
    iconB.style.left = '68%'; iconB.style.top = '48%'; iconB.style.transform = 'translate(-50%,-50%)'; iconB.style.opacity = '1';
    fx.appendChild(iconA); fx.appendChild(iconB);
    requestAnimationFrame(function(){
      requestAnimationFrame(function(){
        iconA.style.left = '50%'; iconA.style.transform = 'translate(-50%,-50%) scale(0.25) rotate(220deg)'; iconA.style.opacity = '0';
        iconB.style.left = '50%'; iconB.style.transform = 'translate(-50%,-50%) scale(0.25) rotate(-220deg)'; iconB.style.opacity = '0';
      });
    });
    setTimeout(function(){
      var burst = document.createElement('div');
      burst.className = 'lab-fx-burst';
      burst.style.animationPlayState = 'running';
      fx.appendChild(burst);
    }, 950);
    setTimeout(function(){
      fx.classList.remove('active');
      state.inventory[result.id] = (state.inventory[result.id]||0) + 1;
      saveState(); renderTop(); renderInventory(); renderLab();
      openModal(result, true);
    }, 1550);
  }

  document.getElementById('lab-combine-btn').addEventListener('click', function(){
    if (!labSlots.a || !labSlots.b) return;
    if (state.balance < LAB_COST){ showToast('Не хватает звёзд'); return; }
    var idA = labSlots.a, idB = labSlots.b;
    var needed = {}; needed[idA] = (needed[idA]||0)+1; needed[idB] = (needed[idB]||0)+1;
    for (var id in needed){ if ((state.inventory[id]||0) < needed[id]){ showToast('Не хватает гифтов'); return; } }
    var gA = byId(idA), gB = byId(idB);
    state.balance -= LAB_COST;
    Object.keys(needed).forEach(function(id2){ state.inventory[id2] -= needed[id2]; });
    var resultRarity = pickFusionRarity(gA, gB);
    var result = pickGiftOfRarity(resultRarity);
    labSlots.a = null; labSlots.b = null;
    saveState(); renderTop(); renderInventory(); renderLab();
    playLabFusion(gA, gB, result);
  });

  // ---------- cases & packs ----------
  var CASES = [
    {id:'bronze',   name:'Бронзовый кейс',   ico:'📦', priceStars:150,  priceCoins:75,  weights:{common:55, rare:32, epic:11, legend:1.7, mythic:0.3}},
    {id:'silver',   name:'Серебряный кейс',  ico:'🥈', priceStars:320,  priceCoins:160, weights:{common:38, rare:38, epic:19, legend:4,   mythic:1}},
    {id:'gold',     name:'Золотой кейс',     ico:'🥇', priceStars:600,  priceCoins:300, weights:{common:18, rare:34, epic:33, legend:11,  mythic:4}},
    {id:'platinum', name:'Платиновый кейс',  ico:'💠', priceStars:950,  priceCoins:475, weights:{common:6,  rare:20, epic:38, legend:28,  mythic:8}},
    {id:'diamond',  name:'Алмазный кейс',    ico:'💎', priceStars:1500, priceCoins:750, weights:{common:1,  rare:8,  epic:28, legend:42,  mythic:21}}
  ];
  var PREMIUM_CASES = [
    {id:'shadow',    name:'Теневой кейс',    ico:'🖤', priceCoins:950,  weights:{rare:4, epic:24, legend:46, mythic:26}},
    {id:'celestial', name:'Небесный кейс',   ico:'🌌', priceCoins:1500, weights:{epic:14, legend:44, mythic:42}}
  ];
  var PACKS = [
    {id:'starter', name:'Стартовый пак',  ico:'🎒', priceStars:110,  priceCoins:55,  items:['rose','bear','heart']},
    {id:'party',   name:'Вечеринка',      ico:'🎉', priceStars:340,  priceCoins:170, items:['cake','balloon','giftbox']},
    {id:'style',   name:'Стильный пак',   ico:'🕶️', priceStars:520,  priceCoins:260, items:['sunglasses','hat','crown']},
    {id:'ocean',   name:'Морской пак',    ico:'🌊', priceStars:720,  priceCoins:360, items:['trident','anchor','crystal']},
    {id:'royal',   name:'Королевский пак',ico:'👑', priceStars:1250, priceCoins:625, items:['cup','diamond','amulet']}
  ];
  var PREMIUM_PACKS = [
    {id:'legend_vault', name:'Легендарное хранилище', ico:'🏆', priceCoins:1100, items:['eagle','unicorn','cup']},
    {id:'mythic_vault',  name:'Мифическое хранилище',  ico:'🐉', priceCoins:2400, items:['dragon','phoenix']}
  ];

  function pickFromWeights(weightsObj){
    var keys = Object.keys(weightsObj);
    var total = keys.reduce(function(s,k){ return s+weightsObj[k]; }, 0);
    var r = Math.random()*total;
    for (var i=0;i<keys.length;i++){ r -= weightsObj[keys[i]]; if (r<=0) return keys[i]; }
    return keys[keys.length-1];
  }

  function openCase(caseObj, currency){
    var price = currency === 'coins' ? caseObj.priceCoins : caseObj.priceStars;
    if (currency === 'coins'){
      if (state.coins < price){ showToast('Не хватает монет'); return; }
      state.coins -= price;
    } else {
      if (state.balance < price){ showToast('Не хватает звёзд'); return; }
      state.balance -= price;
    }
    var rarity = pickFromWeights(caseObj.weights);
    var result = pickGiftOfRarity(rarity);
    state.inventory[result.id] = (state.inventory[result.id]||0) + 1;
    saveState(); renderTop(); renderInventory(); renderBank();
    openModal(result, true);
  }

  function openPack(packObj, currency){
    var price = currency === 'coins' ? packObj.priceCoins : packObj.priceStars;
    if (currency === 'coins'){
      if (state.coins < price){ showToast('Не хватает монет'); return; }
      state.coins -= price;
    } else {
    ...
    var results = packObj.items.map(function(id){ return byId(id); }).filter(Boolean);
    results.forEach(function(g){ state.inventory[g.id] = (state.inventory[g.id]||0) + 1; });
    saveState(); renderTop(); renderInventory(); renderBank();
    openBatchModal(results);
  }

  function renderCasesAndPacks(){
    var casesGrid = document.getElementById('cases-grid');
    casesGrid.innerHTML = '';
    CASES.forEach(function(c){
      var card = document.createElement('div');
      card.className = 'shop-card';
      card.innerHTML = '<div class="ico">'+c.ico+'</div><div class="name">'+c.name+'</div>';
      var btnWrap = document.createElement('div');
      btnWrap.style.display = 'flex'; btnWrap.style.gap = '6px'; btnWrap.style.marginTop = '10px';
      var btnS = document.createElement('button');
      btnS.className = 'btn-secondary'; btnS.style.flex = '1'; btnS.textContent = c.priceStars + ' ★';
      btnS.addEventListener('click', function(){ openCase(c, 'stars'); });
      var btnC = document.createElement('button');
      btnC.className = 'btn-secondary gold'; btnC.style.flex = '1'; btnC.textContent = c.priceCoins + ' 🪙';
      btnC.addEventListener('click', function(){ openCase(c, 'coins'); });
      btnWrap.appendChild(btnS); btnWrap.appendChild(btnC);
      card.appendChild(btnWrap);
      casesGrid.appendChild(card);
    });

    var premCasesGrid = document.getElementById('premium-cases-grid');
    premCasesGrid.innerHTML = '';
    PREMIUM_CASES.forEach(function(c){
      var card = document.createElement('div');
      card.className = 'shop-card';
      card.innerHTML = '<div class="ico">'+c.ico+'</div><div class="name">'+c.name+'</div>';
      var btn = document.createElement('button');
      btn.className = 'btn-secondary gold'; btn.style.width = '100%'; btn.style.marginTop = '10px';
      btn.textContent = c.priceCoins + ' 🪙';
      btn.addEventListener('click', function(){ openCase(c, 'coins'); });
      card.appendChild(btn);
      premCasesGrid.appendChild(card);
    });

    var packsGrid = document.getElementById('packs-grid');
    packsGrid.innerHTML = '';
    PACKS.forEach(function(p){
      var card = document.createElement('div');
      card.className = 'shop-card';
      card.innerHTML = '<div class="ico">'+p.ico+'</div><div class="name">'+p.name+'</div>';
      var btnWrap = document.createElement('div');
      btnWrap.style.display = 'flex'; btnWrap.style.gap = '6px'; btnWrap.style.marginTop = '10px';
      var btnS = document.createElement('button');
      btnS.className = 'btn-secondary'; btnS.style.flex = '1'; btnS.textContent = p.priceStars + ' ★';
      btnS.addEventListener('click', function(){ openPack(p, 'stars'); });
      var btnC = document.createElement('button');
      btnC.className = 'btn-secondary gold'; btnC.style.flex = '1'; btnC.textContent = p.priceCoins + ' 🪙';
      btnC.addEventListener('click', function(){ openPack(p, 'coins'); });
      btnWrap.appendChild(btnS); btnWrap.appendChild(btnC);
      card.appendChild(btnWrap);
      packsGrid.appendChild(card);
    });

    var premPacksGrid = document.getElementById('premium-packs-grid');
    premPacksGrid.innerHTML = '';
    PREMIUM_PACKS.forEach(function(p){
      var card = document.createElement('div');
      card.className = 'shop-card';
      card.innerHTML = '<div class="ico">'+p.ico+'</div><div class="name">'+p.name+'</div>';
      var btn = document.createElement('button');
      btn.className = 'btn-secondary gold'; btn.style.width = '100%'; btn.style.marginTop = '10px';
      btn.textContent = p.priceCoins + ' 🪙';
      btn.addEventListener('click', function(){ openPack(p, 'coins'); });
      card.appendChild(btn);
      premPacksGrid.appendChild(card);
    });
  }

  // ---------- telegram login ----------
  // Юзернейм твоего бота (без @) — нужен только для входа через обычный браузер (виджет).
  var TG_BOT_USERNAME = 'gametbf_bot';
  var ADMIN_USERNAMES = ['TBFPUMBA_ADMIN'];
  var ADMIN_IDS = [];
  function isAdmin(){
    if (!state.tgProfile) return false;
    if (state.tgProfile.id && ADMIN_IDS.indexOf(state.tgProfile.id) !== -1) return true;
    if (state.tgProfile.username && ADMIN_USERNAMES.indexOf(state.tgProfile.username) !== -1) return true;
    return false;
  }

  // ---------- maintenance mode (shared via a JSON file in this repo, always the same for everyone) ----------
  var MAINT_REPO_OWNER = 'TBFPUMBA';
  var MAINT_REPO_NAME = 'GIFT-BATLEE-TBF';
  var MAINT_BRANCH = 'main';
  var MAINT_FILE = 'maintenance.json';
  var MAINT_RAW_URL = 'https://raw.githubusercontent.com/' + MAINT_REPO_OWNER + '/' + MAINT_REPO_NAME + '/' + MAINT_BRANCH + '/' + MAINT_FILE;

  function fetchMaintenanceState(cb){
    fetch(MAINT_RAW_URL + '?t=' + Date.now(), {cache:'no-store'}).then(function(r){
      if (!r.ok) throw 0; return r.json();
    }).then(function(data){ cb(data && typeof data.on === 'boolean' ? data : {on:false}); })
      .catch(function(){ cb({on:false}); });
  }
  function buildMaintEditUrl(on, message){
    var json = JSON.stringify({on:on, message:message||''}, null, 2);
    return 'https://github.com/' + MAINT_REPO_OWNER + '/' + MAINT_REPO_NAME + '/new/' + MAINT_BRANCH +
      '?filename=' + encodeURIComponent(MAINT_FILE) + '&value=' + encodeURIComponent(json);
  }
  function showMaintenanceScreen(message){
    document.body.innerHTML =
      '<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:30px;background:#050709;color:#eaf1fb;font-family:\'IBM Plex Mono\',monospace;">' +
      '<div style="font-size:60px;margin-bottom:16px;">🛠️</div>' +
      '<div style="font-family:\'Anton\',sans-serif;font-size:26px;text-transform:uppercase;margin-bottom:10px;">Игра на техническом перерыве</div>' +
      '<div style="color:#7d92b8;font-size:14px;max-width:420px;">' + (message || 'Скоро вернёмся, спасибо за терпение.') + '</div>' +
      '</div>';
  }

  // Если сайт открыт как мини-приложение внутри Telegram — профиль подтягивается
  // напрямую через нативный Web App SDK, без всяких переходов в Chrome.
  ...
  function renderTgProfile(){
    ...
    if (state.tgProfile){
      ...
      chip.innerHTML =
        ...
        (state.tgFromMiniApp ? '' : '<button class="tg-logout" id="tg-logout-btn" title="Выйти">✕</button>') +
        '</div>';
      if (!state.tgFromMiniApp){
    ...
    renderProfileCard();
    document.getElementById('side-menu-admin').style.display = isAdmin() ? 'flex' : 'none';
  }
  ...
  var ACCENT_THEMES = {
    ...
    white:  {blue:'#eaf1fb', cta:'#ffffff', dim:'#8a97ab'},
    pink:   {blue:'#ec4899', cta:'#f472b6', dim:'#831843'},
    teal:   {blue:'#14b8a6', cta:'#2dd4bf', dim:'#0f766e'}
  };
  ...
  function playRaritySound(rarity){
    if (rarity === 'mythic'){ playTone(660,0.12); setTimeout(function(){playTone(880,0.12);},110); setTimeout(function(){playTone(1180,0.25);},220); }
    else if (rarity === 'legend'){ playTone(600,0.1); setTimeout(function(){playTone(900,0.2);},100); }
    else if (rarity === 'epic'){ playTone(520,0.15); }
    else if (rarity === 'rare'){ playTone(420,0.12); }
    else { playTone(320,0.09); }
  }
  ...
  function applySettings(){
    ...
    document.getElementById('haptics-toggle').checked = !!state.settings.haptics;
    document.getElementById('compact-toggle').checked = !!state.settings.compact;
    document.getElementById('boost-alerts-toggle').checked = state.settings.boostAlerts !== false;
  ...
  document.getElementById('sound-toggle').addEventListener('change', function(e){
    ...
    if (e.target.checked){ playTone(500,0.1); }
  });
  document.getElementById('haptics-toggle').addEventListener('change', function(e){
    state.settings.haptics = e.target.checked;
    saveState(); applySettings();
    if (e.target.checked){ vibrate(30); }
  });
  document.getElementById('compact-toggle').addEventListener('change', function(e){
    state.settings.compact = e.target.checked;
    saveState(); applySettings();
  });
  document.getElementById('boost-alerts-toggle').addEventListener('change', function(e){
    state.settings.boostAlerts = e.target.checked;
    saveState();
  });
  document.getElementById('reset-progress-btn').addEventListener('click', function(){
    if (!window.confirm('Сбросить весь прогресс? Звёзды, монеты, гифты, уровень — всё удалится безвозвратно.')) return;
  ...
  function openSideMenu(){
    ...
    var tgHost = document.getElementById('side-menu-tg');
    if (state.tgProfile){
      var p = state.tgProfile;
      tgHost.innerHTML = (p.photo_url ? '<img src="'+p.photo_url+'" alt="">' : '<div class="side-menu-avatar-ph">👤</div>') +
        '<span>'+p.first_name+(p.last_name?' '+p.last_name:'')+'</span>';
    } else {
      tgHost.innerHTML = '<div class="side-menu-avatar-ph">👤</div><span>Гость</span>';
    }
  }
  ...
  document.querySelectorAll('.side-menu-item').forEach(function(btn){
    btn.addEventListener('click', function(){
      var action = btn.getAttribute('data-menu-action');
      closeSideMenu();
      if (action === 'profile'){ switchScreen('profile'); }
      else if (action === 'settings'){ setTimeout(openSettings, 260); }
      else if (action === 'admin'){ setTimeout(openAdminPanel, 260); }
    });
  });

  var adminModal = document.getElementById('admin-modal');
  function openAdminPanel(){
    fetchMaintenanceState(function(data){
      document.getElementById('maint-toggle').checked = !!data.on;
      document.getElementById('maint-message').value = data.message || '';
    });
    adminModal.classList.remove('hidden');
    requestAnimationFrame(function(){ adminModal.classList.add('visible'); });
  }
  function closeAdminPanel(){
    adminModal.classList.remove('visible');
    setTimeout(function(){ adminModal.classList.add('hidden'); }, 200);
  }
  document.getElementById('admin-close').addEventListener('click', closeAdminPanel);
  adminModal.addEventListener('click', function(e){ if (e.target === adminModal) closeAdminPanel(); });
  document.getElementById('maint-save').addEventListener('click', function(){
    var on = document.getElementById('maint-toggle').checked;
    var msg = document.getElementById('maint-message').value;
    window.open(buildMaintEditUrl(on, msg), '_blank');
    showToast('Открыл GitHub — нажми зелёную «Commit changes»');
    closeAdminPanel();
  });

  document.addEventListener('keydown', function(e){
    if (e.key !== 'Escape') return;
    if (!adminModal.classList.contains('hidden')){ closeAdminPanel(); return; }
    if (sideMenu.classList.contains('active')){ closeSideMenu(); return; }
  ...
  fetchMaintenanceState(function(maint){
    if (maint.on && !isAdmin()){ showMaintenanceScreen(maint.message); return; }
    applySettings();
    renderTgProfile();
    checkReferralParam();
    renderReferral();
    ...
    function checkBoostExpiry(){
      if (!state.settings.boostAlerts) return;
      var now = Date.now();
      Object.keys(state.boosts).forEach(function(id){
        var expiresAt = state.boosts[id];
        if (!expiresAt || expiresAt <= now) return;
        var msLeft = expiresAt - now;
        if (msLeft <= 5*60*1000 && !alertedBoosts[id]){
          alertedBoosts[id] = true;
          var b = boostById(id);
          if (b) showToast('⏳ «' + b.name + '» закончится через 5 минут');
        }
        if (msLeft > 5*60*1000){ alertedBoosts[id] = false; }
      });
    }
    setInterval(function(){ chargeBankUpkeep(); renderTop(); checkBoostExpiry(); if (currentScreen === 'bank'){ renderBank(); } }, 60*1000);
  });
})();
</script>
</body>
